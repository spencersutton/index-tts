...
class AccelInferenceEngine:
    ...
    def generate(
        ...
        tts_embeddings: torch.Tensor,  # TTS: [pad][cond][text] embeddings (87 tokens, NO start_mel)
        ...
        is_varlen_batch = (
            True and True and batch_size > 1 and (attention_mask.sum(dim=1) != attention_mask.size(1)).any()
        )
        ...
        for i, token_id in enumerate(first_token_list):
            if stop_tokens and token_id in stop_tokens:
                ...
                generated_tokens[i].append(int(token_id))
                sequences[i].append_token(int(token_id))
        ...
        if all(is_finished):
            ...
            for i in range(batch_size):
                full_sequence = list(map(int, input_ids[i].tolist() + generated_tokens[i]))
