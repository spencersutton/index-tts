    ...
    def _parse_audio_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=lambda hf_inputs: dict(audio_embeds=MultiModalFieldConfig.batched("audio")),
            )

        # 如果传入了 "audio" 但不是期望的字典格式，就报错
        raise TypeError(
            f"For 'audio' modality, expected a dict like {'{'} 'audio_embeds': tensor {'}'}, but got {type(data)}"
        )
...
class GPT2TTSMultiModalProcessor(BaseMultiModalProcessor[GPT2TTSProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        def get_replacement(item_idx: int):
            ...
            return PromptUpdateDetails.select_token_id([PLACEHOLDER_TOKEN_ID] * num_features, PLACEHOLDER_TOKEN_ID)
...
@MULTIMODAL_REGISTRY.register_processor(
    GPT2TTSMultiModalProcessor, info=GPT2TTSProcessingInfo, dummy_inputs=GPT2TTSDummyInputsBuilder
)
class GPT2TTSModel(nn.Module, SupportsPP, SupportsMultiModal):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.transformer = GPT2Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, "transformer"))
        ...
        self.lm_head = ParallelLMHead(
            self.config.vocab_size, self.config.n_embd, quant_config=quant_config, prefix=f"{prefix}.lm_head", bias=True
        )
