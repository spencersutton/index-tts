...
class UnifiedVoice(nn.Module):
    def __init__(
        ...
        del self.gpt.wte
        self.mel_pos_embedding, self.text_pos_embedding = (
    ...
    async def inference_speech(
        ...
        mel_start_emb = self.mel_embedding(
            torch.full(
                (
                    emb.shape[0],
                    1,
        ...
        tokens_prompt = TokensPrompt(
            prompt=fake_inputs, multi_modal_data=multi_modal_data
        )
        ...
        output_generator = self.llm.generate(
            tokens_prompt, sampling_params=self.sampling_params, request_id=request_id
        )
        gpt_stt = time.time()
        prefill_flag = True
        async for output in output_generator:
            if prefill_flag:
                logger.info(
                    f"[{request_id}] [prefill time: {(time.time() - gpt_stt):.4f}]"
                )
                gpt_stt = time.time()
                prefill_flag = False
        logger.info(
            f"[{request_id}] [decode time: {(time.time() - gpt_stt):.4f}] [decode len: {len(output.outputs[0].token_ids)}]"
        )
        codes = output.outputs[0].token_ids[:-2]
        codes = torch.tensor(
            codes, device=text_inputs.device, dtype=torch.long
        ).unsqueeze(0)
