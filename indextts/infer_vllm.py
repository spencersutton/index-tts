...
def trim_and_pad_silence(wav_data, threshold=1000, min_silence=int(24000 * 0.4)):
    ...
    last_non_silent = len(abs_trimmed) - np.argmax(
        abs_trimmed[::-1] >= threshold
    )  # 最后一个≥threshold的索引+1
...
class IndexTTS:
    def __init__(
        ...
        self.bpe_path = os.path.join(
            self.model_dir, "bpe.model"
        )  # self.cfg.dataset["bpe_model"]
    ...
    def remove_long_silence(
        self, codes: list, latent: torch.Tensor, max_consecutive=15, silent_token=52
    ):
        assert latent.dim() == 3 and latent.size(0) == 1, (
            "Latent should be (1, seq_len, dim)"
        )
            ...
            with torch.no_grad():
                ...
                latent = self.gpt(
                    ...
                    cond_mel_lengths=torch.tensor(
                        [speech_conditioning_latent.shape[-1]],
                ...
                m_start_time = time.perf_counter()
                wav, _ = self.bigvgan(
                    latent, [ap_.transpose(1, 2) for ap_ in auto_conditioning]
                )
    ...
    async def infer_with_ref_audio_embed(self, speaker: str, text):
        ...
        speech_conditioning_latent = self.speaker_dict[speaker][
            "speech_conditioning_latent"
        ]
