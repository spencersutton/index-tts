    ...
    def __init__(
        ...
        if self.use_cuda_kernel:
            ...
            try:
                ...
                logger.info(
                    f">> Preload custom CUDA kernel for BigVGAN {anti_alias_activation_cuda}"
                )
            except Exception:
                traceback.print_exc()
                logger.info(
        ...
        semantic_code_ckpt = os.path.join(
            self.model_dir, "semantic_codec/model.safetensors"
        )
        ...
        # )
        campplus_ckpt_path = os.path.join(
            self.model_dir, "campplus/campplus_cn_common.bin"
        )
...
class QwenEmotion:
    ...
    async def inference(self, text_input):
        ...
        output_generator = self.model.generate(
            tokens_prompt, sampling_params=sampling_params, request_id=uuid.uuid4().hex
        )
