...
def normalize(x: np.ndarray) -> np.ndarray:
    """Normalize an array to have max absolute value of 1.0."""
...
def mel_spectrogram(
    ...
    This function uses slaney norm for the mel filterbank (using torchaudio.functional.melscale_fbanks) and uses Hann window for STFT (using torch.stft).
    ...
    Args:
        ...
        fmax (int): Maximum frequency for mel filterbank. If None, defaults to half the sampling rate (fmax = sr / 2.0)
    ...
    if key not in mel_basis_cache:
        ...
        mel_basis_cache[key] = mel.T.float().to(device)
