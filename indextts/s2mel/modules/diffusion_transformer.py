...
class DiT(torch.nn.Module):
    ...
    def forward(self, x, prompt_x, x_lens, t, style, cond, mask_content=False):
        ...
        if (
            self.transformer_style_condition and not self.style_as_token
        ):  # True and True
            x_in = torch.cat(
                [x_in, style[:, None, :].repeat(1, T, 1)], dim=-1
            )  # [2, 1863, 864]
        ...
        x_res = self.transformer(
            x_in, t1.unsqueeze(1), input_pos, x_mask_expanded
        )  # [2, 1863, 512]
        ...
        if self.final_layer_type == "wavenet":
            ...
            x = self.wavenet(x, x_mask, g=t2.unsqueeze(2)).transpose(
                1, 2
            ) + self.res_projection(x_res)  # long residual connection
