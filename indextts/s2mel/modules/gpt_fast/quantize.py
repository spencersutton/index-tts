...
def pack_scales_and_zeros(scales, zeros):
    ...
    return (
        torch
        .cat([scales.reshape(scales.size(0), scales.size(1), 1), zeros.reshape(zeros.size(0), zeros.size(1), 1)], 2)
        .transpose(0, 1)
        .contiguous()
    )
...
class GPTQQuantHandler(QuantHandler):
    ...
    def get_inputs(
        ...
        input_recorder = InputRecorder(model, tokenizer, calibration_seq_length, pad_calibration_inputs)
        ...
        evaluate(input_recorder, task_dict, limit=calibration_limit)
    ...
    def create_quantized_state_dict(
        ...
        GPTQ_runner = GenericGPTQRunner(self.mod, inputs, blocksize, percdamp, groupsize).configure_quantization_mode(
