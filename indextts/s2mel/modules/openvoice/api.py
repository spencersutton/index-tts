...
class BaseSpeakerTTS(OpenVoiceBaseClass):
    language_marks = {"english": "EN", "chinese": "ZH"}
        ...
        for t in texts:
            ...
            with torch.no_grad():
                ...
                audio = (
                    self.model
                    .infer(
                        x_tst, x_tst_lengths, sid=sid, noise_scale=0.667, noise_scale_w=0.6, length_scale=1.0 / speed
                    )[0][0, 0]
                    .data.cpu()
                    .float()
                    .numpy()
    ...
    def convert(self, src_waves, src_wave_lengths, src_se, tgt_se, tau=0.3, message="default"):
        ...
        with torch.no_grad():
            ...
            audio = self.model.voice_conversion(
                spec, spec_lengths, sid_src=src_se.unsqueeze(-1), sid_tgt=tgt_se.unsqueeze(-1), tau=tau
            )[0]
        return audio
