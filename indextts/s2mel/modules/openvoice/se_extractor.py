...
def split_audio_whisper(audio_path, audio_name, target_dir="processed"):
    ...
    # create directory
    pathlib.Path(target_folder).mkdir(exist_ok=True, parents=True)
    wavs_folder = os.path.join(target_folder, "wavs")
    pathlib.Path(wavs_folder).mkdir(exist_ok=True, parents=True)
...
def split_audio_vad(audio_path, audio_name, target_dir, split_seconds=10.0):
    ...
    segments = get_vad_segments(
        audio_vad, output_sample=True, min_speech_duration=0.1, min_silence_duration=1, method="silero"
    )
