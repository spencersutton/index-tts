...
def _laplace_smoothing(x: Tensor, n_categories: int, eps: float = 1e-5) -> Tensor:
...
def _kmeans(
    samples: Tensor, num_clusters: int, num_iters: int = 10, use_cosine_sim: bool = False
) -> tuple[Tensor, Tensor]:
...
class _EuclideanCodebook(nn.Module):
    ...
    def replace(self, samples: Tensor, mask: Tensor) -> None:
