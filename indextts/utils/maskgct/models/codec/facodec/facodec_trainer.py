...
from modules.dac.nn.loss import FocalLoss, L1Loss, MelSpectrogramLoss, MultiScaleSTFTLoss
...
from models.codec.facodec.modules.commons import build_model, load_checkpoint, load_F0_models, log_norm
...
class FAcodecTrainer(CodecTrainer):
    def __init__(self, args, cfg):
        ...
        with self.accelerator.main_process_first():
            ...
            if args.resume_type:
                ...
                self.checkpoints_path = json.load(Path(os.path.join(ckpt_path, "ckpts.json")).open())
    ...
    def _build_optimizer(self):
        scheduler_params = {"warmup_steps": self.cfg.loss_params.warmup_steps, "base_lr": self.cfg.loss_params.base_lr}
    ...
    def train_loop(self):
        ...
        while self.epoch < self.max_epoch:
            ...
            for key, loss in train_losses.items():
                ...
                self.accelerator.log({f"Epoch/Train {key} Loss": loss}, step=self.epoch)
            self.accelerator.log({"Epoch/Train Total Loss": train_total_loss}, step=self.epoch)
            ...
            if self.accelerator.is_main_process and save_checkpoint:
                ...
                save_path = os.path.join(
                    self.checkpoint_dir, "FAcodec_epoch_%05d_step_%05d.pth" % (self.epoch, self.iters)
                )
                torch.save(state, save_path)
                json.dump(
                    self.checkpoints_path,
                    Path(os.path.join(self.checkpoint_dir, "ckpts.json")).open("w"),
                    ensure_ascii=False,
                    indent=4,
                )
        ...
        self.accelerator.wait_for_everyone()
        if self.accelerator.is_main_process:
            path = os.path.join(self.checkpoint_dir, f"epoch-{self.epoch:04d}_step-{self.step:07d}")
            print("Saving..")
            state = {
                "net": {key: self.model[key].state_dict() for key in self.model},
                "optimizer": self.optimizer.state_dict(),
            ...
            save_path = os.path.join(self.checkpoint_dir, "FAcodec_epoch_%05d_step_%05d.pth" % (self.epoch, self.iters))
            torch.save(state, save_path)
        ...
        for batch in tqdm(
            ...
            if self.batch_count % self.cfg.train.gradient_accumulation_step == 0:
                ...
                for key, _ in losses.items():
                    self.accelerator.log({f"Step/Train {key} Loss": losses[key]}, step=self.step)
    ...
    def _train_step(self, data):
        ...
        if self.cfg.preprocess_params.frame_rate != 80:
            f0_targets = F.interpolate(
                f0_targets.unsqueeze(1), mel_seg_len // 80 * self.cfg.preprocess_params.frame_rate, mode="nearest"
            ).squeeze(1)
            w2v_seg = F.interpolate(w2v_seg, mel_seg_len // 80 * self.cfg.preprocess_params.frame_rate, mode="nearest")
        ...
        content_loss = self.criterions["content"](
            pred_content.transpose(1, 2)[..., :common_min_size], target_content_latents.long()
        )
        rev_content_loss = (
            self.criterions["content"](
                rev_pred_content.transpose(1, 2)[..., :common_min_size], target_content_latents.long()
            )
        ...
        if resume_type == "resume":
            if checkpoint_path is None:
                ...
                if (
                    ...
                    Path(earliest_checkpoint).unlink()
