...
class Backbone(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Input tensor of shape (B, C, L), where B is the batch size,
                        C denotes output features, and L is the sequence length.

        Returns:
            torch.Tensor: Output of shape (B, L, H), where B is the batch size, L is the sequence length,
