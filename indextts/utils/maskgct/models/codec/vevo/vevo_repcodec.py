...
class VectorQuantize(nn.Module):
    ...
    def __init__(self, dim: int, codebook_size: int, decay=0.8, commitment=1.0, eps=1e-5, n_embed=None):
        super().__init__()
    ...
    def __init__(
        ...
        if stride == 1:
            ...
            self.conv = ConvTranspose1d(
                in_channels=in_channels, out_channels=out_channels, kernel_size=(2 * stride), stride=stride, bias=bias
            )
...
class VevoRepCodec(nn.Module):
    def __init__(
        ...
        self.projector = Projector(
            input_channels=self.encoder.out_channels, code_dim=code_dim, kernel_size=3, stride=1, bias=False
        )
