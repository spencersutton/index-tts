...
import patch_vllm  # ⚠️ Monkey Patch, do not delete this line  # noqa: F401
...
async def run_single_inference(llm: AsyncLLM, inputs_embeds: torch.Tensor):
    ...
    output_generator = llm.generate(
        tokens_prompt, sampling_params, request_id=request_id
    )
...
async def run_user_simulation(
    llm: AsyncLLM, inputs_embeds: torch.Tensor, num_runs: int = 10
) -> List[Dict[str, Any]]:
    user_results = []
...
async def benchmark(llm_engine, concurrency_levels, runs_per_user):
    ...
    fake_inputs_embeds = torch.randn(
        1, conds_len + max_text_tokens, hidden_dim, dtype=torch.float16, device="cpu"
    )
