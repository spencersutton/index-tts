...
def load_language_list(language):
    with pathlib.Path(os.path.join(I18N_JSON_DIR, f"{language}.json")).open("r", encoding="utf-8") as f:
...
class I18nAuto:
    def __init__(self, language=None):
        ...
        if not pathlib.Path(os.path.join(I18N_JSON_DIR, f"{language}.json")).exists():
            language = "en_US"
