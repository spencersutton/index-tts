...
def get_kernel_metadata(
    node_schedule: Union[Sequence[BaseSchedulerNode], ExternKernel], wrapper: PythonWrapperCodegen
) -> tuple[str, str]: ...
def dominated_nodes(
    initial_queue: Iterable[torch.fx.Node], skip_filter: Optional[Callable[[Any], bool]] = ...
...
def fresh_cache(
    cache_entries: Optional[dict[str, Any]] = ..., dir: Optional[str] = ..., delete: bool = ...
) -> Iterator[None]: ...
...
def get_tma_workspace_arg(
    num_tma_descriptors: int, device: torch.device, num_programs: Optional[int] = ...
) -> WorkspaceArg: ...
...
def is_fallback_op(
    node: Optional[Operation], op: Union[torch._ops.OpOverload, Collection[torch._ops.OpOverload]]
) -> bool: ...
...
def copy_misaligned_inputs(
    new_inputs: list[InputType], check_inputs_idxs: Sequence[int], return_pair_idxs: Optional[OrderedSet[int]] = ...
...
def register_op_dtype_propagation_rules(
    name: str, type_promotion_kind: ELEMENTWISE_TYPE_PROMOTION_KIND, override_return_dtype: Optional[torch.dtype]
) -> None: ...
...
def maybe_log_cudagraph_partition(
    msg: str, prefix: Optional[str] = ..., node: Optional[BaseSchedulerNode] = ...
) -> None: ...
