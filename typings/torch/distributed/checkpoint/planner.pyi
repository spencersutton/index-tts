...
class BytesIOWriteData:
    """BytesIOWriteData(nbytes: int)"""

    nbytes: int

@dataclass(frozen=True)
class TensorWriteData:
    """TensorWriteData(chunk: torch.distributed.checkpoint.metadata.ChunkStorageMetadata, properties: torch.distributed.checkpoint.metadata.TensorProperties, size: torch.Size)"""
...
@dataclass(frozen=True)
class ReadItem:
    """ReadItem(type: torch.distributed.checkpoint.planner.LoadItemType, dest_index: torch.distributed.checkpoint.metadata.MetadataIndex, dest_offsets: torch.Size, storage_index: torch.distributed.checkpoint.metadata.MetadataIndex, storage_offsets: torch.Size, lengths: torch.Size)"""
...
@dataclass(frozen=True)
class SavePlan:
    """SavePlan(items: list[torch.distributed.checkpoint.planner.WriteItem], storage_data: Any = None, planner_data: Any = None, usable: bool = True)"""
...
@dataclass
class LoadPlan:
    """LoadPlan(items: list[torch.distributed.checkpoint.planner.ReadItem], storage_data: Any = None, planner_data: Any = None)"""
